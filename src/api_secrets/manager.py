...
# Encryption key will be loaded at runtime from environment


def _get_cipher():
    """Get the encryption cipher"""
    try:
        # Always get key from environment to allow for runtime changes
        encryption_key = os.environ.get("ENCRYPTION_KEY")
        if not encryption_key:
            # Generate a proper default key for development/testing
            encryption_key = Fernet.generate_key().decode()
            print(
                f"Warning: Using generated encryption key. Set ENCRYPTION_KEY environment variable in production."
            )

        # Fernet expects the base64-encoded key as bytes
        return Fernet(encryption_key.encode())
    except Exception as e:
        print(f"Error initializing cipher: {e}")
        print(
            f"Encryption key from env: {os.environ.get('ENCRYPTION_KEY', 'NOT_SET')[:20]}..."
        )
        return None
