...
import { createRetryStrategy, executeWithRetry } from './retry/retryStrategy';
import { findStepById, validateExecutionEnvironment } from './stepUtils';
import { executeStepCommand, executeConnectStep } from './commandExecution';
...
export const executeDeploymentStep = async (
  ...
  const finalRetryStrategy = createRetryStrategy(retryStrategy);
  ...
  try {
    return await executeWithRetry(
      ...
      async (attemptNumber) => {
        ...
        if (attemptNumber > 0) {
          addLog(`[${step.title}] - Retry attempt ${attemptNumber + 1} of ${finalRetryStrategy.maxAttempts}...`, 'info');
        }
